	...
	while(!((shoulderDone && armDone) || joy2Btn(7))) { //While the motors are not done and the operator is not overriding
		if(!shoulderDone) { //Test if the shoulder is done before changing it
			if(!shoulderDown && nMotorEncoder[ShoulderLeft] < shoulderTarget) { //If we are still going to our target, set the motor speed based on a function
				motor[ShoulderLeft] = motor[ShoulderRight] = shoulderScale * targetMotorSpeed(shoulderTotal, nMotorEncoder[ShoulderLeft]);
			}
			else if(shoulderDown && nMotorEncoder[ShoulderLeft] > shoulderTarget) {
				motor[ShoulderLeft] = motor[ShoulderRight] = -shoulderScaleDown * targetMotorSpeed(shoulderTotal, nMotorEncoder[ShoulderLeft]);
			}
		...
		if(!armDone) { //Same thing for the arm
			if(!armDown && nMotorEncoder[ArmLeft] < armTarget) {
				motor[ArmLeft] = motor[ArmRight] = armScale * targetMotorSpeed(armTotal, nMotorEncoder[ArmLeft]);
			}
			else if(armDown && nMotorEncoder[ArmLeft] > armTarget) {
				motor[ArmLeft] = motor[ArmRight] = -armScaleDown * targetMotorSpeed(armTotal, nMotorEncoder[ArmLeft]);
			}
			else {
...
void forward(byte encoderCount) {
	...
	motor[LeftForward] = motor[RightForward] = 100; //Turn the motors on
